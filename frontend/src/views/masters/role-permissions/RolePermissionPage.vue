...
const togglePermission = async (item: RolePermissionItem, nextActive: boolean) => {
  ...
  try {
    ...
    showToast(`Permission ${item.resource}:${item.action} ${nextActive ? 'aktif' : 'nonaktif'}`)
  }
  catch (error) {
    showToast(getErrorMessage(error), 'error')
  }
...
<template>
  <VCard>
    <VCardItem>
      <template #title>
        <div class="d-flex align-center justify-space-between flex-wrap gap-4">
          <span class="text-h6">Role Permission Setting</span>
          <VChip
            color="primary"
            label
          >
            Active: {{ activeCount }} / {{ permissions.length }}
          </VChip>
        </div>
      </template>
    </VCardItem>
    ...
    <VCardText>
      ...
      >
        Role aktif: <strong>{{ selectedRoleName }}</strong>
      </div>
    </VCardText>
  </VCard>
