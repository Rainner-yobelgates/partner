...
const submitForm = async () => {
  ...
  try {
    if (editedItem.value) {
      ...
      showToast('Driver berhasil dibuat')
      page.value = 1
    }
...
const confirmDelete = async () => {
  ...
  try {
    ...
    showToast('Driver berhasil dihapus')
...
<template>
    ...
    <VCardItem>
      <template #title>
        <div class="d-flex align-center justify-space-between flex-wrap gap-4">
          <span class="text-h6">Master Driver</span>
      ...
      <VTable density="comfortable">
        ...
        <tbody>
          <tr v-if="!isLoading && rows.length === 0">
            <td colspan="7" class="text-center text-medium-emphasis py-6">Data driver belum ada.</td>
          </tr>
  ...
  <VDialog v-model="isFormDialogOpen" max-width="720">
    <VCard>
      <VCardItem :title="isEditMode ? 'Edit Driver' : 'Tambah Driver'" />
      <VCardText>
        <VForm @submit.prevent="submitForm">
          <VRow>
            ...
            <VCol cols="12" md="6">
              <VTextField v-model="form.emergency_contact" label="Emergency Contact" />
            </VCol>
            <VCol cols="12" md="6">
              <VSelect v-model="form.type" label="Tipe Driver" :items="['MAIN', 'ASSISTANT', 'RESERVE']" />
            </VCol>
            <VCol cols="12" md="6">
              <VSelect
  ...
  <VDialog v-model="isDeleteDialogOpen" max-width="420">
    <VCard>
      <VCardItem title="Hapus Driver" />
      <VCardText>Yakin hapus driver <strong>{{ editedItem?.name }}</strong>?</VCardText>
      <VCardActions class="justify-end">
