...
const search = ref('')
const sortBy = ref<'created_at' | 'origin'>('created_at')
...
<template>
  <VCard>
    ...
    <VCardText>
      <VRow>
        ...
        <VCol cols="6" md="2"><VSelect v-model="sortBy" label="Urutkan" :items="[{ title: 'Dibuat', value: 'created_at' },{ title: 'Asal', value: 'origin' },]" item-title="title" item-value="value" /></VCol>
    ...
    <VCardText>
      ...
      <VTable density="comfortable">
        <thead>
          <tr><th>Asal</th><th>Tujuan</th><th>Jarak (km)</th><th>Estimasi (menit)</th><th>Status</th><th>Dibuat Pada</th><th class="text-end">Aksi</th></tr>
        </thead>
        <tbody>
          <tr v-if="!isLoading && rows.length===0"><td colspan="7" class="text-center text-medium-emphasis py-6">Data rute belum ada.</td></tr>
  ...
  <VDialog v-model="isFormDialogOpen" max-width="700">
    <VCard>
      <VCardItem :title="isEditMode ? 'Ubah Rute' : 'Tambah Rute'" />
      <VCardText>
        <VForm @submit.prevent="submitForm">
          <VRow>
            <VCol cols="12" md="6"><VTextField v-model="form.origin" label="Asal" /></VCol>
            <VCol cols="12" md="6"><VTextField v-model="form.destination" label="Tujuan" /></VCol>
            <VCol cols="12" md="6"><VTextField v-model.number="form.distance" label="Jarak (km)" type="number" /></VCol>
            <VCol cols="12" md="6"><VTextField v-model.number="form.estimated_time" label="Estimasi Waktu (menit)" type="number" /></VCol>
